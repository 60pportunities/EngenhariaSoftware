...
- [ ] Se há implantação tensa, há o medo;
- [ ] O medo corrói a confiança, sem confiança, não nos sentimos seguros, a única motivação que nos resta é a da autopreservação;
 - [ ] Eles começaram a pensar demais em seu código e a não cumprir seus compromissos. Isso não era bom para o produto;
- [ ] E não era bom para a equipe;
- [ ] Sem mencionar o fato de que isso criou uma tensão doentia entre nossa Equipe de Liderança Executiva e todos os outros;
- [ ] Vamos imaginar: Refatorar uma consulta de banco de dados SQL;
```
...
- [ ] Como identificar as equipes necessárias?
- [ ] Como avisar as equipes necessárias, acordando-as se necessário?
- [ ] Como sair efetuando o escalonamento?
- [ ] Como iniciar a chamada de Zoom ou Teams em uma "sala de guerra"?
- [ ] Como comunicar efetivamente o status da interrupção e o tempo estimado para correção?
- [ ] Como elaborar Análise de Causa Raiz (Root Cause Analysis - RCA)?
	- [ ] Cada RCA incluía uma discussão técnica sobre o que aconteceu, como tomamos conhecimento do problema, como identificamos a causa raiz e as etapas que pretendíamos tomar para evitar que ocorresse novamente.
- [ ] Como criar uma equipe Change Control Board (CCB)?
	- [ ] Deve revisar e aprovar todas as mudanças feitas no produto com a esperança de que decisões, causadoras de interrupções, sejam pegas antes que causem um problema.
...
- [ ] **Implementação controlada** : as equipes podem lançar um novo recurso primeiro para um pequeno
grupo e depois para todos;
- [ ] **Soluções rápidas** : se algo quebrar, as equipes podem desativar o recurso sem reimplantar
todo o sistema.
- [ ] **Teste A/B**: os sinalizadores de recursos ajudam a testar diferentes versões de um recurso para ver
qual funciona melhor;
- [ ] **Atualizações do servidor**: os sinalizadores de recursos permitem que as equipes testem as alterações em alguns servidores antes de atualizar todos;
- [ ] **Controle de custos da nuvem**: as empresas os utilizam para desativar serviços extras quando não necessário;
- [ ] **Recursos de segurança**: as equipes podem implementar novas configurações de segurança passo a passo, verificando se há problema.
...
## Gerundiando
   - [ ] Implantação
   - [ ] Implantar é o ato de enviar código para um servidor de produção.
   - [ ] "Implantado" se refere apenas à localização do código, não à sua participação no fluxo de controle do aplicativo. O código atrás de um sinalizador de recurso e nenhum usuário estiver sendo visado, então nenhum usuário estará realmente executando esse código implantado.
   - [ ] Liberação
   - [ ] Liberar é o ato de expor código e funcionalidade implantados para seus usuários.
...
- [ ] Um sinalizador de recurso de produto deve ser usado durante o desenvolvimento de um novo recurso de produto. Os sinalizadores de recurso de produto devem ter um cronograma finito. Um sinalizador de recurso de "produto" nunca será lançado para 100% dos usuários;
- [ ] Os sinalizadores de recursos de operações, por outro lado, estão vinculados à operação contínua do próprio aplicativo e não têm cronograma inerente. Não há intenção de remover esse sinalizador de recurso do código.
### Variantes
...
- [ ] Imediato, o código implantado é liberado para todos os usuários ao mesmo tempo.
- [ ] Gradual, o código implantado é liberado para um número crescente de usuários ao longo do tempo.
## Roll-back
...
- [ ] Sinalizador de recurso esteja ativado somente para usuários com um endereço de e-mail interno da empresa.
- [ ] Sinalizador de recurso seja habilitado apenas para uma pequena porcentagem de usuários.

## Implantação
Imagine uma implementação muito mais graduada usando sinalizadores de recursos:

- [ ] Implante código inativo em servidores de produção — nenhum usuário vê qualquer diferença ou impacto inicial no aplicativo.
- [ ] Habilitar sinalizador de recurso para seu ID de usuário .
- [ ] Recurso de teste em produção.
- [ ] Descubra um bug.
- [ ] Corrija o bug e reimplante o código (ainda ativo apenas para seu usuário).
- [ ] Examine os logs de erros.
- [ ] Habilitar sinalizador de recurso para usuários internos da empresa .
- [ ] Examine registros de erros e métricas.
- [ ] Descobrir bug(s).
- [ ] Corrigir bugs e reimplementar o código (ainda ativo apenas para usuários internos da empresa).
- [ ] Habilitar sinalizador de recurso para 10% de todos os usuários.
- [ ] Examine registros de erros e métricas.
- [ ] Habilitar sinalizador de recurso para 25% de todos os usuários.
- [ ] Examine registros de erros e métricas.
- [ ] Habilitar sinalizador de recurso para 50% de todos os usuários.
- [ ] Examine registros de erros e métricas.
- [ ] Habilitar sinalizador de recurso para 75% de todos os usuários.
- [ ] Examine registros de erros e métricas.
- [ ] Habilitar sinalizador de recurso para todos os usuários.
- [ ] Comemore um lançamento de recurso bem-sucedido.
...
- [ ] **Controle simples**: Ative ou desative recursos sem alterar o código;
- [ ] **Testes Inteligentes**: Experimente coisas novas sem quebrar o sistema;
- [ ] **Melhores Decisões**:Use dados reais antes de tornar os recursos permanentes;
